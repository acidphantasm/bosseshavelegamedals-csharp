...
public class ConfigProvider(LegaMedalsModConfig config) : IConfigEditorConfigProvider
{
    public IEnumerable<ConfigEditorConfigRegistration> GetConfigs()
    {
        ...
        metadata.Name,
        config,
        Path.Combine("user", "mods", "acidphantasm-bosseshavelegamedals", "config.json")
        );
    }
}